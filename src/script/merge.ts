...
import { warn, defaultOptions, configFileExists, log } from '../utils'
import { getAllExcel } from '../i18n-file'
import fs from 'fs'

function merge(range: string[] | 'all'): void {
  ...
  const existsExcel = getAllExcel(path.join(root, excel))

  // 检测文件是否存在
  if (range !== 'all' && range.some((name) => !existsExcel.includes(name))) {
    warn('non-existent excel file is specified.')
  }

  // 合并所有 或者指定合并中必须合并到 i18n.xlsx 中
  const needMergeFiles = range === 'all' ? existsExcel : range
  if (needMergeFiles.length <= 1) {
    warn('Must specify two or more excel files.')
  }

  const cache = new Map<string, { file: string; [props: string]: string }[]>()
  ...
  for (const file of needMergeFiles) {
    ...
    for (let i = 1; i < data.length; i++) {
      // 获取每一行的对象格式
      const o: { file: string; [props: string]: string } = { file }
      for (let j = 0; j < first.length; j++) {
        if (j === zhIndex) continue
        o[first[j]] = data[i][j]
      }
...
function gen(
  cache: Map<string, { file: string; [props: string]: string }[]>,
  type: string[],
  root: string,
  excel: string,
  files: string[]
  ...
  for (const [zh, info] of cache.entries()) {
    ...
    for (let i = 0; i < type.length; i++) {
      if (type[i] === 'zh') tmp.push(zh)
      else tmp.push(info[0][type[i]])
    }
    data.push(tmp)
  }
  ...
  fs.writeFileSync(path.resolve(root, excel, name), new Uint8Array(buffer), {
    flag: 'w',
  })

  for (const file of files) {
    fs.unlinkSync(path.resolve(root, excel, file))
  }

  log(`Merge finished, ${name}`)
  process.exit(0)
}
